package lava.walkinggroup.proxy;
...
public class ProxyBuilder {
    ...
    public static <T extends Object> void callProxy(Call<T> caller, final SimpleCallback<T> callback) {
        callProxy(null, caller, callback, null);
    }
     ...
     * @param caller    Call object returned by the proxy
     * @param callback  Client-code to execute when we have a good answer for them.
     * @param onErrorCallback  Client-code to execute when connection error with server.
     * @param <T>       The type of data that Call object is expected to fetch
     */
    public static <T extends Object> void callProxy(Call<T> caller, final SimpleCallback<T> callback, final SimpleCallback<Void> onErrorCallback) {
        callProxy(null, caller, callback, onErrorCallback);
    }
     ...
     * - Callback to simplified interface on success.
     * - Callback to optional function when error occurs.
    ...
    public static <T extends Object> void callProxy(
        ...
        callProxy(context, caller, callback, null);
    }
    ...
    public static <T extends Object> void callProxy(
            final Context context, Call<T> caller,
            final SimpleCallback<T> callback,
            final SimpleCallback<Void> onErrorCallback) {

        CurrentSession now = CurrentSession.getInstance();
        ...
        caller.enqueue(new Callback<T>() {
            ...
            public void onResponse(Call<T> call, retrofit2.Response<T> response) {
                ...
                if (response.errorBody() == null) {
                    ...
                    if (callback != null) {
                        now.setConnectionStatus(CurrentSession.ConnectionStatus.CONNECTED);
            ...
            private void showFailure(String message) {
                ...
                // TODO: We should have a system to indicate errors, I propose an alternate, optional callback which is called if error occurs.
                now.setConnectionStatus(CurrentSession.ConnectionStatus.FAILED);
            }
        });
    }
